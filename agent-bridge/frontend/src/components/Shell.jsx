...
const SECTIONS = ['System', 'Setup', 'Agent']

const STATUS_MAP = {
  ready:         { color: 'var(--c-green)', label: 'Ready',            dot: true  },
  partial:       { color: 'var(--c-amber)', label: 'Setup incomplete', dot: true  },
  unconfigured:  { color: 'var(--c-red)',   label: 'Not configured',   dot: false },
}

// Which nav items have readiness tied to them
const ITEM_FLAG = {
...
export default function Shell() {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'var(--sidebar-w) 1fr', gridTemplateRows: 'var(--topbar-h) 1fr', height: '100vh', overflow: 'hidden' }}>
      ...
      <header style={{
        ...
        <div style={{
          ...
          <span style={{
            ...
            boxShadow: systemStatus === 'ready' ? `0 0 0 0 ${sm.color}` : 'none',
            ...
            {NAV.filter(n => n.section === sec).map(n => {
              ...
              return (
                <NavLink key={n.to} to={n.to}
                  ...
                  onMouseEnter={e => { if (!e.currentTarget.className) e.currentTarget.style.background = 'var(--c-raised)' }}
                >
                  {({ isActive }) => (<>
      ...
      {/* ── Main content ────────────────────────────────────────────────── */}
      <main style={{ overflowY: 'auto', padding: '32px 36px', background: 'var(--c-void)' }}>
...
function Breadcrumb({ path }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: 'var(--t-lo)' }}>
      ...
      <span style={{ color: 'var(--t-mid)', fontWeight: 500 }}>{label}</span>
    </div>
  )
}
